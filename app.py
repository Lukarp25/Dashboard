...
industry_filter = st.sidebar.multiselect(
    "Select Industry",
    options=sorted(master_isin["Industry"].unique()),
    default=sorted(master_isin["Industry"].unique())
)
...
# ── Apply Fund filter at the top level ──────────────────
filtered_df = df[
    (df["Fund"].isin(fund_filter)) &
...
def calculate_monthly_change(data):
    data = data.sort_values(["AMC", "Fund", "ISIN", "Month_dt"])
    data["Prev_Quantity"] = data.groupby(["AMC", "Fund", "ISIN"])["Quantity"].shift(1)
...
# Fund type color map for charts that split by Fund
fund_color_map = {
    "MIDCAP": "#1f77b4",
    "SMALL":  "#ff7f0e"
}
...
with tab1:
    ...
    split_by_fund_tab1 = st.toggle("Split bars by Fund Type", value=False, key="fund_split_tab1")

    holdings_df = (
        filtered_df[filtered_df["Month"] == selected_month_1]
        .groupby(["AMC", "Fund", "ISIN", "Share Name", "Industry"])["%_to_NAV"]
        .sum()
        .reset_index()
        .sort_values("%_to_NAV", ascending=False)
    )

    color_col = "Fund" if split_by_fund_tab1 else "AMC"
    color_map  = fund_color_map if split_by_fund_tab1 else amc_color_map
    ...
    holdings_ind_df = (
        filtered_df[filtered_df["Month"] == selected_month_1]
        .groupby(["AMC", "Fund", "Industry"])["%_to_NAV"]
        .sum()
        .reset_index()
        .sort_values("%_to_NAV", ascending=False)
    )
...
with tab2:
    ...
    with col1:
        month_1 = st.selectbox(
            "Select Base Month", months_sorted,
            index=1 if len(months_sorted) > 1 else 0, key="month1"
        )
    with col2:
        month_2 = st.selectbox(
            "Select Comparison Month", months_sorted,
            index=0, key="month2"
        )
    ...
    # Keep Fund in groupby so we can track fund-level changes
    df_m1 = df_m1.groupby(["AMC", "Fund", "ISIN"])["Quantity"].sum().reset_index()
    df_m2 = df_m2.groupby(["AMC", "Fund", "ISIN"])["Quantity"].sum().reset_index()
    ...
    merged_df["Per_Change"] = merged_df.apply(
        ...
        if row["Quantity_m1"] != 0 else 0, axis=1
    )
    merged_df = merged_df.merge(master_isin, on="ISIN", how="left")

    split_by_fund_tab2 = st.toggle("Split bars by Fund Type", value=False, key="fund_split_tab2")
    color_col2 = "Fund" if split_by_fund_tab2 else "AMC"
    color_map2  = fund_color_map if split_by_fund_tab2 else amc_color_map
    ...
    fig_inc = px.bar(
        ...
        x="Share Name", y="Per_Change",
        color=color_col2, text="Per_Change",
        color_discrete_map=color_map2,
        title="Top Increases in Holdings (%)"
    )
    ...
    fig_dec = px.bar(
        ...
        color=color_col2, text="Per_Change",
        color_discrete_map=color_map2,
        title="Top Decreases in Holdings (%)"
    )
...
with tab6:
    ...
    # Show Fund breakdown for this stock as a label
    funds_holding = view2_df["Fund"].unique()
    st.caption(f"Fund type(s) holding this stock: **{', '.join(sorted(funds_holding))}**")

    fig2 = px.bar(
        view2_df,
        x="Month", y="Per_Change",
        color="AMC", text="Per_Change",
        color_discrete_map=amc_color_map,
        facet_row="Fund" if len(funds_holding) > 1 else None
    )
    ...
    fig7 = px.line(
        ...
        x="Month", y="Quantity",
        color="AMC",
        line_dash="Fund",          # distinguishes MIDCAP vs SMALL visually
        color_discrete_map=amc_color_map
    )
...
with tab3:
    ...
    selected_month_2 = st.selectbox(
        "Select Month for Analysis", months_sorted,
    ...
    st.dataframe(new_additions[["Fund", "AMC", "Share Name", "Industry", "Quantity", "%_to_NAV"]])
    ...
    if not previous_month_row.empty:
        ...
        # Key on AMC + Fund + ISIN so a stock removed from one fund but
        # still held in another shows correctly
        prev_keys = set(zip(prev_month_df["AMC"], prev_month_df["Fund"], prev_month_df["ISIN"]))
        curr_keys = set(zip(curr_month_df["AMC"], curr_month_df["Fund"], curr_month_df["ISIN"]))
        removed_keys = prev_keys - curr_keys

        removed_df = prev_month_df[
            list(zip(prev_month_df["AMC"], prev_month_df["Fund"], prev_month_df["ISIN"]))
            .apply(lambda r: r in removed_keys, axis=None)
            if False else
            prev_month_df.apply(
                lambda row: (row["AMC"], row["Fund"], row["ISIN"]) in removed_keys, axis=1
            )
        ]

        st.dataframe(removed_df[["Fund", "AMC", "Share Name", "Industry", "%_to_NAV"]])
...
with tab4:
    ...
    with col1:
        ...
        st.dataframe(
            most_bought[["Fund", "Share Name", "AMC", "Change"]],
            height=table_height, width='stretch'
        )
    with col2:
        st.subheader("Top Sold")
        st.dataframe(
            most_sold[["Fund", "Share Name", "AMC", "Change"]],
            height=table_height, width='stretch'
        )
...
with tab5:
    ...
    # ── Overlap scope: across all selected funds, or per-fund ────
    overlap_scope = st.radio(
        "Overlap Scope",
        ["All selected funds combined", "Per Fund Type"],
        horizontal=True, key="overlap_scope"
    )
    ...
    def build_overlap_heatmap(overlap_data, title):
        pivot_qty = (
            overlap_data.pivot_table(
                index="ISIN", columns="AMC",
        ...
        for amc1 in amcs:
            for amc2 in amcs:
                ...
                overlap_matrix.loc[amc1, amc2] = round((min_sum / max_sum) * 100, 2) if max_sum else 0
        ...
        fig_ov.update_layout(height=650, font=dict(size=14), title_font_size=20)
        return fig_ov, pivot_qty

    overlap_base = filtered_df[filtered_df["Month"] == selected_month_4]

    if overlap_scope == "All selected funds combined":
        fig_ov, pivot_qty = build_overlap_heatmap(overlap_base, "AMC Overlap (%) — All Funds")
        ...
        overlap_stocks_df = overlap_base[overlap_base["ISIN"].isin(overlap_isins)]
        pivot_table = (
            overlap_stocks_df.pivot_table(
                index=["Share Name", "Industry"],
                columns="AMC", values="%_to_NAV", aggfunc="sum"
        ...
        for fund_name in sorted(overlap_base["Fund"].unique()):
            fund_data = overlap_base[overlap_base["Fund"] == fund_name]
            if fund_data.empty:
                continue
            st.markdown(f"### Fund: {fund_name}")
            fig_ov, pivot_qty = build_overlap_heatmap(fund_data, f"AMC Overlap (%) — {fund_name}")
            st.plotly_chart(fig_ov, width='stretch')

            overlap_counts = (pivot_qty > 0).sum(axis=1)
            overlap_isins  = overlap_counts[overlap_counts >= 2].index
            overlap_stocks_df = fund_data[fund_data["ISIN"].isin(overlap_isins)]
            if not overlap_stocks_df.empty:
                pivot_table = (
                    overlap_stocks_df.pivot_table(
                        index=["Share Name", "Industry"],
                        columns="AMC", values="%_to_NAV", aggfunc="sum"
...
# =========================================================
# TAB 7 - INDUSTRY LEVEL MONTHLY CHANGE
# =========================================================
with tab7:
    ...
    with col1:
        month_1_ind = st.selectbox(
            "Select Base Month", months_sorted,
            index=1 if len(months_sorted) > 1 else 0, key="month1_industry"
        )
    with col2:
        month_2_ind = st.selectbox(
            "Select Comparison Month", months_sorted,
            index=0, key="month2_industry"
        )

    df_m1_ind = filtered_df[filtered_df["Month"] == month_1_ind]
    df_m2_ind = filtered_df[filtered_df["Month"] == month_2_ind]

    # Include Fund in groupby
    df_m1_ind = df_m1_ind.groupby(["AMC", "Fund", "Industry"])["Quantity"].sum().reset_index()
    df_m2_ind = df_m2_ind.groupby(["AMC", "Fund", "Industry"])["Quantity"].sum().reset_index()

    merged_ind_df = pd.merge(
        df_m1_ind, df_m2_ind,
        on=["AMC", "Fund", "Industry"],
        how="outer", suffixes=("_m1", "_m2")
    ).fillna(0)

    merged_ind_df["Change"] = merged_ind_df["Quantity_m2"] - merged_ind_df["Quantity_m1"]
    merged_ind_df["Per_Change"] = merged_ind_df.apply(
        lambda row: (row["Change"] / row["Quantity_m1"] * 100)
        if row["Quantity_m1"] != 0 else 0, axis=1
    )

    split_by_fund_tab7 = st.toggle("Split bars by Fund Type", value=False, key="fund_split_tab7")
    color_col7 = "Fund" if split_by_fund_tab7 else "AMC"
    color_map7  = fund_color_map if split_by_fund_tab7 else amc_color_map

    # ── INCREASE ────────────────────────────────────────
    st.subheader(f"Increase in Industry Holdings ({month_1_ind} → {month_2_ind})")
    top_n_inc_ind = st.slider("Select Top N Industry Increases", 5, 50, 20, key="inc_slider_industry")

    increase_ind_df = merged_ind_df[merged_ind_df["Change"] > 0].sort_values("Per_Change", ascending=False)

    fig_inc_ind = px.bar(
        increase_ind_df.head(top_n_inc_ind),
        x="Industry", y="Per_Change",
        color=color_col7, text="Per_Change",
        color_discrete_map=color_map7,
        title="Top Increases in Industry Holdings (%)"
    )
    fig_inc_ind.update_traces(texttemplate='<b>%{text:.2f}%</b>', textposition="outside")
    fig_inc_ind.update_layout(height=650, xaxis_tickangle=-45, font=dict(size=14))
    st.plotly_chart(fig_inc_ind, width='stretch')

    # ── DECREASE ────────────────────────────────────────
    st.subheader(f"Decrease in Industry Holdings ({month_1_ind} → {month_2_ind})")
    top_n_dec_ind = st.slider("Select Top N Industry Decreases", 5, 50, 20, key="dec_slider_industry")

    decrease_ind_df = merged_ind_df[merged_ind_df["Change"] < 0].sort_values("Per_Change")
    ...
    fig_dec_ind.update_traces(texttemplate='<b>%{text:.2f}%</b>', textposition="outside")
    fig_dec_ind.update_layout(height=650, xaxis_tickangle=-45, font=dict(size=14))
...
with tab8:
    ...
    with col1:
        base_month = st.selectbox(
            "Base Month", months_sorted,
            index=1 if len(months_sorted) > 1 else 0, key="watch_base"
        )
    with col2:
        curr_month = st.selectbox(
            "Current Month", months_sorted,
            index=0, key="watch_curr"
        )
    ...
    # Keep Fund so we can count cross-fund conviction
    df_m1 = df_m1.groupby(["AMC", "Fund", "ISIN"])["Quantity"].sum().reset_index()
    df_m2 = df_m2.groupby(["AMC", "Fund", "ISIN"])["Quantity"].sum().reset_index()
    ...
    stock_level = buying_df.groupby("ISIN").agg(
        ...
        Avg_Per_Change=("Per_Change", "mean"),
        Buying_AMC_Count=("AMC", "nunique"),
        Funds_Active=("Fund", lambda x: ", ".join(sorted(x.unique())))   # new: which fund types
    ).reset_index()
    ...
    stock_level["Score"] = (
        ...
        (stock_level["Avg_Per_Change"].rank(pct=True) * 0.25) +
    ...
    st.dataframe(
        final_df.head(top_n)[[
            "Share Name", "Industry", "Funds_Active",
            "Score", "Buying_AMC_Count", "Total_Change", "Avg_Per_Change", "New_Entry"
        ]],
        width='stretch'
    )
    ...
    fig = px.bar(
        final_df.head(top_n),
        x="Share Name", y="Score",
        color="Industry",
        title="Top Consensus Buys",
        hover_data=["Funds_Active"]
    )
